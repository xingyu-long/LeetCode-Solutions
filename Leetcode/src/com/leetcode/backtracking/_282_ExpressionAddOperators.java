...
public class _282_ExpressionAddOperators {
    ...
    public void dfs(List<String> res, String num, int target, String path, int pos, long val, long pre) {
        // iterate fully.
        if (pos == num.length()) {
        ...
        for (int i = pos; i < num.length(); i++) {
            ...
            if (pos == 0) { // 不能加入 * - + 操作
                dfs(res, num, target, path + cur, i + 1, cur, cur);
            } else {
                dfs(res, num, target, path + "+" + cur, i + 1, val + cur, cur);
                dfs(res, num, target, path + "-" + cur, i + 1, val - cur, -cur);
                dfs(res, num, target, path + "*" + cur, i + 1, (val - pre) + pre * cur, pre * cur);
            }
        }
    }
}
